...
public class HTTPClient {
    ...
    public HTTPClient(String host) {
        this.host = host;
        ADD = String.format("http://%s/add", host);
    ...
    public String add(Serializable request, String xaId) {
        try {
            String response = restTemplate.postForObject(ADD, request, String.class);
            xaToBookingIds.put(xaId, response);
            logger.info("SUCCESS {} request='{}' response='{}', xaId={}", ADD, request, response, xaId);
            return response;
        } catch (Exception e) {
            logger.error("ERROR {} request='{}' exception='{}', xaId={}", ADD, request, e.getMessage(), xaId);
            throw e;
        }
    }
